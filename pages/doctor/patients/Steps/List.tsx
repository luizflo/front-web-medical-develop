...
export default function List({
  ...
  const handleEvent = (id: any) => {
    const pacientSelected = listPatients.filter((item) => item.id === id);
    setPatientSelected(pacientSelected[0]);
    onClick();
      ...
      >
        {props.professionals.length > 0 ? (
          ...
          <EmptyComponent
            ...
            type={"other"}
            title={"Ainda não há pacientes no seu registro"}
            message={
  ...
  return (
    <Grid className={styles.container}>
      <FeedBack
        ...
        message={feedBack}
      />
      <Header length={listPatients.length} handleOpen={handleOpen} />

      <TableHeader />
      <TableHistory
        isListLoading={loading}
        professionals={listPatients}
        handleClick={onClick}
